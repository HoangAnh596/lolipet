@section('title', 'Thông kê đơn hàng bị hủy')
        ...
        <div class="card card-white my-0">
            <div class="card-header">
                <ol class="breadcrumb float-sm-left ">
                    <li class="breadcrumb-item card-title">Thống kê đơn hàng bị hủy</li>
                </ol>
            </div>
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
...
$(document).ready(function() {
    ...
    const data = {
        labels: months,
        datasets: [{
            label: 'Thống kê tổng đơn bị hủy hàng theo tháng',
            data: datas,
    ...
    const config = {
        ...
        options: {
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Biểu đồ thống kê đơn hàng bị hủy',
                    font: {
                        size: 14
                    }
                }
    ...
    $('#time').change(function(e) {
        ...
        $.ajax({
            url: "{{ route('statistical.orderCancel') }}",
            type: 'GET',
            data: {
                time: $('#time').val()
            },
